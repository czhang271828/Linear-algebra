...
> 选用 T1 的函数, 记 $\mu(x) := \frac{a(x)}{e}$. 假定 $\delta = \Phi (\varphi)$, 则
>
> 1. 一方面 ($\delta$ 的定义), $\int_{\mathbb R}\mu(nx)\varphi (x) \operatorname dx = \mu(0) = 1$.
> 2. 另一方面, $|\int_{\mathbb R} \mu(nx)\varphi (x)\operatorname dx| \leq \int_{-1/n}^{1/n} |\varphi (x)|\operatorname dx \xrightarrow {n \to \infty} 0$.
>
> 矛盾.
...
> $\int_{\mathbb R} f \cdot \Phi (f) \operatorname dx \geq 0$, 取等当且仅当 $f=0$. 因此 $\Phi(f) = 0$ 当且仅当 $f=0$.
...
> $\delta$ 是对偶空间 $V^\ast$ 中的元素, 定义作
> $$
> \delta : V \to \mathbb R,\quad f \mapsto f(0).
> $$
> 
...
> 对任意 $f \in V$,
> $$
> \int_{\mathbb R}f(x)\varphi'' (x) \operatorname dx  := \int_{\mathbb R}f''(x)\varphi(x) \operatorname dx .
> $$
> 分段计算,
>
> 1. $\int_{-1}^02(x+1)f''(x) \operatorname dx  = 2f(-1)-2f(0)+2f'(0)$.
> 2. $\int_0^1(2-3x)f''(x) \operatorname dx = -3f(0)+3f(1)-2f'(0)-f'(1)$.
> 3. $\int_1^2 (x-2)f''(x) \operatorname dx = f(1) - f(2) + f'(1)$.
>
> 相加, 得
> $$
> \int_{\mathbb R}f''(x) \varphi(x):= 2 f(-1)-5f(0)+4f(1)-f(2).
> $$
> 因此, 在 $V^{\ast}$ 中定义
> $$
> \varphi '' (x):  2\delta(x+1)-5\delta(x)+4\delta(x-1)-\delta(x-2).
> $$
> 当然, 这一答案也能猜到, 例如 $\varphi'$ 是阶梯函数, 二阶导数是阶梯高度差, 用 $\delta$ 表示即可.
...
> 改变参考系. 记 $f^n(x) := f(\frac x n)$​, 此时
> $$
> \int_{-\infty}^\infty f^n(x)\varphi(x)\, \mathrm{d}x =  \int_{-\infty}^\infty f(x)\varphi_n(x)\, \mathrm{d}x.
> $$
> 不妨设 $\varphi$ 仅在 $[-1,1]$ 区间外恒零. 由 $0$ 点处的连续性, $f^n$ 在 $[-1,1]$ 一致收敛至常函数 $f(0)$. 因此
> $$
> L(f) = f(0)\cdot \int_{\mathbb R}\varphi (x) \operatorname dx.
> $$
> 依照定义, 这是线性的.
