...
7. Demonstrate that if any two elements of $\{f,g,g\circ f\}$ belong to $\mathrm{AI}(V)$, then so does the third.
   ...
   > *Hints:* Show that 
   >
   > 1.  $[\ker g \circ f \to \ker g,\quad x \mapsto f(x)]$ has kernel $\ker f$; 
   > 2. the dual statement of $\uparrow$; 
   > 3. for $f \in \mathrm{AI}$, set $\Phi (f) := \dim \ker f - \dim \frac{V}{\mathrm{im}(f)}$, show that $\Phi (f\circ g) = \Phi(f) + \Phi (g)$.
