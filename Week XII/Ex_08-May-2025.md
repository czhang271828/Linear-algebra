...
> [!NOTE]
>
> In this diagonalisable case, one may also verify that $\mathrm{rank}(A) = \mathrm{rank}(A\overline A)$; that is, the multiplicity of $0$ is preserved. For $A = \binom {0 \ \ 1}{0 \ \ 0}$, $A\overline A = O$ is diagonalisable yet $A$ is not cdiagonalisable.
...
> [!note]
>
> $A$ may not csimilar to an Hermite matrix, e.g., $A = \binom{0 \ \ 1}{i \ \ 0}$.
