...
\begin{remark}
    答案见\href{http://proofwiki.org/wiki/Inverse_of_Hilbert_Matrix}{此文}. 若矩阵通过两个数列和的倒数表达 (即, $m_{i,j}=(a_i+b_j)^{-1}$), 则称之 Hilbert 矩阵. Hilbert 矩阵虽然不是 $u^Tv$ 形式的秩 $1$ 矩阵, 但
    \begin{equation}
...
\begin{remark}
    答案见\href{http://proofwiki.org/wiki/Inverse_of_Vandermonde_Matrix}{此文}. 以上矩阵称作 Vandermonde 矩阵, 其主要用途是构造可逆矩阵或是线性无关组. Vandermonde 矩阵的种种性质不依赖基的选取, 有限域上的 Vandermonde 矩阵与编码理论紧密关联. 
\end{remark}
