...
1. $\mathbb F[x] = \{\text{polynomials over $\mathbb F$}\} = \{\text{$\mathbb F$-sequences with finite non-zeros}\}$; 
2. $\mathbb F[\![x]\!] = \{\text{formal power series over $\mathbb F$}\} = \{\text{$\mathbb F$-sequences}\}$. 
...
2. for $\mathbb F = \mathbb R$, the formal power series $f(x) = \sum_{n\geq 1}x^ne^{n}$ diverges almost everywhere.
