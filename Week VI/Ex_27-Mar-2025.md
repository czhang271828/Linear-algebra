...
7. $\mathbb F[x]$ as a subspace of $\mathrm{Hom}_\mathbb F(\mathbb F[\![x]\!], x)$. 
...
**Exercise 2 (optional if you have done this before)** Show that $U \simeq \mathrm{Hom}_{\mathbb F}(U, \mathrm{Hom}_{\mathbb F}(U, \mathbb F))$ if $\dim U < \infty$. 
...
$$
\mathrm{Hom}_{\mathbb C}(U, V) \simeq (\mathrm{Hom}_{\mathbb R}(U,V))^2\quad (\text{as $\mathbb R$-linear spaces}).
$$
Here $W^2 := W\times W$. 

> **Step 0** Show that $\mathrm{Hom}_{\mathbb C}(U, V)$ is indeed an $\mathbb R$-linear space.  
>
> **Step 1** Write down explicitly the linear map $f \mapsto (f,\overline f)$, to be explicit, 
> $$
> \Phi : \quad \text{Big Formula...}
> $$
> **Step 2** Check that $\Phi$ is a well-defined linear map, i.e., 
